...
export function createApp(httpServer: import('http').Server) {
	...
	const presenceInterval = setInterval(() => cleanupPresence(io), 60000);
		...
		try {
			...
			socket.on('get_presence', (data: { userIds: string[] }) => {
				try {
					...
					if (currentUserId) {
						...
						if (_userData) _userData.lastSeen = Date.now();
					}
				} catch (err) {
			...
			socket.on('friend_request', (data: { targetUserId: string }) => {
				try {
					...
					if (_userData_fr) _userData_fr.lastSeen = Date.now();
			...
			socket.on('friend_accept', (data: { targetUserId: string }) => {
				try {
					...
					if (_userData_fa) _userData_fa.lastSeen = Date.now();
			...
			socket.on('friend_decline', (data: { targetUserId: string }) => {
				try {
					...
					if (_userData_fd) _userData_fd.lastSeen = Date.now();
			...
			socket.on('message', (data: { to: string; ciphertext: string; nonce: string }) => {
				try {
					...
					if (_userData_msg) _userData_msg.lastSeen = Date.now();
			...
			socket.on('messages_read', (data: { from: string; contactId: string }) => {
				try {
					...
					if (_userData_mr) _userData_mr.lastSeen = Date.now();
			...
			socket.on('delete_message', (data: { targetUserId: string; nonces: string[] }) => {
				try {
					...
					if (_userData_dm) _userData_dm.lastSeen = Date.now();
			...
			socket.on('call_accept', (data: { callId: string; sdp: string }) => {
				try {
					...
					if (_userData_ca) _userData_ca.lastSeen = Date.now();
			...
			socket.on('call_decline', (data: { callId: string }) => {
				try {
					...
					if (_userData_cd) _userData_cd.lastSeen = Date.now();
			...
			socket.on('call_hangup', (data: { callId: string }) => {
				try {
					...
					if (_userData_ch) _userData_ch.lastSeen = Date.now();
			...
			socket.on('ice_candidate', (data: { callId: string; candidate: string }) => {
				try {
					...
					if (_userData_ic) _userData_ic.lastSeen = Date.now();
